"""
Интеграционные сценарии agent (живой LLM + браузер + ЕИС).

По умолчанию НЕ гоняются обычным pytest (маркер integration).

Из каталога tools-server:
  py -m pytest app/agent/tests -m integration -v
  py -m pytest app/agent/tests -m integration -k scenario_1 -v
"""
...
from app.agent.loop import run_platform_task
from app.agent.tests.scenarios import SCENARIOS
