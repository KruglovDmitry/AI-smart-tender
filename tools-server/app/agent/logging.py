...
# Avoid clashing with this module's name (`app.agent.logging`).
...
def setup_agent_file_logging(run_tag: str | None = None) -> Path:
    """
    Attach a UTF-8 FileHandler to app.agent / app.core.browser loggers.
    ...
    _logging.getLogger("app.agent").info("agent file log started: %s", path)
...
def log_tool_step(name: str, args: dict, result: object) -> None:
    if not config.AGENT_DEBUG_LOGS:
        return
    logger = _logging.getLogger("app.agent.tools")
...
def log_messages(step_i: int, messages: list[Any], *, label: str = "messages") -> None:
    ...
    if not config.AGENT_DEBUG_LOGS:
        return
    logger = _logging.getLogger("app.agent")
